...
This file holds **cross-cutting rules** that apply everywhere. Per-directory rules live in nested `CLAUDE.md` files (loaded on demand when Claude touches files in that subtree).

## Where to find more guidance

| When you are working in… | Read…                                                                                                 |
| ------------------------ | ----------------------------------------------------------------------------------------------------- |
| `src/`                   | [src/CLAUDE.md](src/CLAUDE.md) — EJS multi-page site, Vite, Tailwind/DaisyUI, the Alpine.js client    |
| `netlify/`               | [netlify/CLAUDE.md](netlify/CLAUDE.md) — the serverless functions: Razorpay deposits + booking lookup |
| `scripts/`               | [scripts/CLAUDE.md](scripts/CLAUDE.md) — the Python image + schedule tooling                          |

Layout / sync contract for these files: [docs/INSTRUCTION_FILES.md](docs/INSTRUCTION_FILES.md).
...
- `src/partials/` — `head.ejs`, `header.ejs`, `footer.ejs`, `analytics-noscript.ejs` (the GTM + Meta Pixel `<noscript>` fallbacks, first thing in every page's `<body>`), plus `reviews.ejs` (a self-contained guest-reviews section any page can include, parameterised via EJS locals) and `whatsapp-float.ejs` (the floating WhatsApp button every guest-facing page includes after its footer). There is **no layout wrapper** — each page assembles them itself.
...
When you change a `CLAUDE.md`, update its `.github/instructions/*.instructions.md` (or `.github/copilot-instructions.md` for the root) mirror **in the same commit**. See [docs/INSTRUCTION_FILES.md](docs/INSTRUCTION_FILES.md).
