...
| `seasonSelector`                    | `packages.ts`         | Season pills on `/packages`. Defaults client-side to the season containing today via the shared calendar in `src/data/season.mjs`; syncs a `#season=` URL hash. Cards render Low-season prices server-side as the no-JS fallback.                                                                       |
| `packageCard(<json>)`               | `packages.ts`         | One card's per-season payload (formatted prices, analytics values, WhatsApp hrefs), built by `src/data/load-packages.mjs` and passed via `x-data`; bindings read `season` from the enclosing `seasonSelector` scope.                                                                                    |
| `packagesAccordion`                 | `packages.ts`         | The "Good to know" accordion on `/packages` — real `<button>`s with `aria-expanded`, one panel open at a time.                                                                                                                                                                                          |
| `packageTracking`                   | `packages.ts`         | Placed on `/packages` CTA anchors to push `package_enquiry` / `view_packages` events, carrying the selected season.                                                                                                                                                                                     |
...
- **Adding gallery images means regenerating that JSON** — see `scripts.instructions.md`. The two galleries have separate files: `gallery-img-metadata.json` and `gallery-shala-img-metadata.json`.
